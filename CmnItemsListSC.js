...
const paymentIdAsString = {
   ...
   134: "BankIslam",
   152: "UOB",
   163: "HongLeongPEx+(QRPayment)",
   166: "BankMuamalat",
   167: "OCBC",
   168: "StandardCharteredBank",
   173: "CIMBVirtualAccount(Delaypayment)",
   ...
   renderItemContents = (item, index) => {
      ...
      switch (item.unitType) {
         ...
         case 'choice': //right just name - choice in 1,2
            output.push(<View key='1' style={{ flex: CmnItemsList.COL1 + CmnItemsList.COL2, alignItems: 'flex-end' }}>
               <GCText list>{`${nameStr} - ${item.choiceText}`}</GCText></View>)
            break;

         case 'yesNo': //right justify name in 1,2
            output.push(<View key='1' style={{ flex: CmnItemsList.COL1 + CmnItemsList.COL2, alignItems: 'flex-end' }}><GCText list>{nameStr}</GCText></View>)
            break;
